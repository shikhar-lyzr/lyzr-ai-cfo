...
| Database | Postgres (Neon) via Prisma | 6.19.3 |
...
```bash
npx prisma generate
npx prisma migrate deploy   # apply committed migrations to the DB in DATABASE_URL
```

For local iteration on a throwaway branch you own, `npx prisma migrate dev` works too. **Never** run `migrate reset` against a shared Neon branch — it drops all data.
...
```env
# Database (Postgres — Neon recommended). Use the pooled URL for the app
# and the direct URL for Prisma Migrate.
...
| **Document** | Markdown reports generated by `save_document` / `generateReport` (`type`: variance_report / ar_summary / close_package). `period?` scopes close-packages to a periodKey |

**Reconciliation & Monthly Close:**

| Model | Purpose |
|---|---|
| **ReconPeriod** | Period (e.g. `2026-04`) registered per user. Unique `(userId, periodKey)`. Source of truth for the period picker. |
| **GLEntry** | Parsed GL row with `periodKey`, `matchStatus` (unmatched/matched/partial) |
| **SubLedgerEntry** | Parsed sub-ledger row, same shape as GL |
| **MatchRun** | One reconciliation pass over GL × sub-ledger for a given periodKey. Holds totals + strategy config |
| **MatchLink** | A successful GL↔sub link inside a MatchRun (strategy + confidence + deltas) |
| **Break** | An unmatched row from a MatchRun. `side` (gl_only / sub_only), `severity`, `ageDays`, `status` (open/resolved) |
| **AdjustmentProposal** | Draft journal entry proposed for a specific Break |
| **JournalAdjustment** | Posted journal adjustment (one-to-one with an approved AdjustmentProposal) |
| **FXRate** | Currency conversion rates used during recon |
...
User ──< DataSource ──< FinancialRecord
  │          │                │
  │          │                ├──< GLEntry ──< MatchLink >── SubLedgerEntry
  │          │                │                   │
  │          │                │           MatchRun >── Break ──< AdjustmentProposal ──< JournalAdjustment
  │          │                │
  ...
  ├──< Document              (type: variance_report | ar_summary | close_package; period? for close_package)
  └──< JournalAdjustment
```
...
| `/api/documents/generate` | POST | Ask the agent to generate a fresh report. Accepts `type` ∈ {`variance_report`, `ar_summary`, `close_package`} + optional `period` (required for `close_package`) |
| `/api/close/periods` | GET | Lightweight list of close periods for the user (union of `ReconPeriod` + `DISTINCT FinancialRecord.period`). Powers the period picker |
| `/api/close/readiness` | GET | `?period=YYYY-MM` → `{ readiness, blockers, tasks }` with per-section error isolation |
| `/api/reconciliation/periods` | GET | Enriched per-period recon stats (match rate, open breaks, …) |
| `/api/reconciliation/breaks` | GET / POST | List breaks / run a match pass for a period |
...
- `2026-04-18-reconciliation-periods-design.md` — Period-aware reconciliation: ReconPeriod model, match runs scoped by periodKey, break ageing
- `2026-04-22-monthly-close-dynamic-design.md` — Monthly Close dynamic flow: readiness score, blockers, 5-card task progress, close-package generation

## Monthly Close

`/monthly-close` is period-aware: `?period=YYYY-MM` URL scopes every metric on the page. All numbers trace to real Prisma rows.

- **Readiness score** (weighted 40/20/20/20): match rate, open-break severity × ageing, required-source freshness, variance anomalies. Tiers: Ready ≥ 85, Caution 60–84, Not Ready < 60. Narrative is template-generated server-side, not an LLM call.
- **Blockers** pulled from existing reconciliation queries + missing required sources + variance anomalies. Variance records are aggregated by category before the threshold check (a quarter has one row per month per category — don't count them separately).
- **5 task cards** computed from real Prisma counts (Sub-ledger Close / GL Entries / Variance Review / Journal Adjustments / Close Package). Empty-state variants route to the action that would fill them.
- **"Explain This"** icons on every metric dispatch `openAskAi` with a pre-built prompt; the chat panel answers with the period pinned in context.
- **Generate Close Package** hits `POST /api/documents/generate` with `{ type: "close_package", period }`, which runs the `close_package` branch of `generateReport` — pulls readiness/blockers/tasks, asks the LLM to draft a structured markdown package, and writes a `Document` row scoped by period.

Source: `app/(shell)/monthly-close/`, `lib/close/`, `app/api/close/`, `agent/skills/monthly-close/SKILL.md`.
